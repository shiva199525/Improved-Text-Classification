...
//  Project2
//
//  Created by Anoja Rajalakshmi on 27/11/16.
//  Copyright © 2016 Anoja. All rights reserved.
//
...
long read_source_file(char* file_path){
    ...
    if (fpp != NULL) {
        ...
        if (fseek(fpp, 0L, SEEK_END) == 0) {
            ...
            file_size = bufsize;
            if (bufsize == -1) { printf("Error-1"); }
            

            source_ptr = malloc(sizeof(char) * (bufsize + 1));
            

            if (fseek(fpp, 0L, SEEK_SET) != 0) { printf("Error-2");}
...
int main(void) {
    ...
    accuracy=(double)(counter/160.0)*100.0;
    printf("\n\nFinal Accuracy ->%f\n",accuracy);
